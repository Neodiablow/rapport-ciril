...
Logstash est un logiciel développé en ruby. Le passage d'une phase à l'autre est 
implémenté via les \emph{SizedQueue}. Elles sont dimensionnées pour contenir 20 
éléments\footnote{appelés messages lorsqu'on parle de queues/fils, on parle bien 
ici des événements logstash, éventuellement encore au stade \textit{d'informations} 
pour la queue pré inputs},% vérifier l'implementation de la queue pré input
ce n'est pas paramétrable c'est un choix délibéré, ces queues ne sont pas conçues 
pour stocker des données à long terme. On verra plus tard que ce la justifie l'utilisation
d'un \textit{buffer}, comme \emph{Redis}.
